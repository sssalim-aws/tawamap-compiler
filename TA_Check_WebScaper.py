...
def get_all_trusted_advisor_checks_in_public_doc():
    result = "N/A"
    try:
        ...
        soup = BeautifulSoup(htmldata, 'html.parser') 
        for data in soup.find_all("div", class_="variablelist"): 
            texts.append(data.get_text())
        for t in texts:
            if "Check ID" in t:                
                split_text = t.split("Check ID")
                x = split_text[1].split("\n")
                id_row = (x[2])
                checkid = id_row.replace("AWS Config ","").replace(" ","")
                cost_check_ids.append(checkid)                    
        output['cost-optimization']=cost_check_ids
        #print(cost_check_ids)

        performance_check_ids = []
        texts = []        
        url = 'https://docs.aws.amazon.com/awssupport/latest/user/performance-checks.html'
        htmldata = getdata(url)
        soup = BeautifulSoup(htmldata, 'html.parser') 
        for data in soup.find_all("div", class_="variablelist"): 
            texts.append(data.get_text())
        for t in texts:
            if "Check ID" in t:                
                ...
                id_row = (x[2])
                checkid = id_row.replace("AWS Config ","").replace(" ","")
                performance_check_ids.append(checkid)                    
        output['performance']=performance_check_ids
        #print(performance_check_ids)

        security_check_ids = []
        texts = []        
        url = 'https://docs.aws.amazon.com/awssupport/latest/user/security-checks.html'
        htmldata = getdata(url)
        soup = BeautifulSoup(htmldata, 'html.parser') 
        for data in soup.find_all("div", class_="variablelist"): 
            texts.append(data.get_text())
        for t in texts:
            if "Check ID" in t:                
                ...
                id_row = (x[2])
                checkid = id_row.replace("AWS Config ","").replace(" ","")
                security_check_ids.append(checkid)                    
        output['security']=security_check_ids
        #print(security_check_ids)

        fault_check_ids = []
        texts = []        
        url = 'https://docs.aws.amazon.com/awssupport/latest/user/fault-tolerance-checks.html'
        htmldata = getdata(url)
        soup = BeautifulSoup(htmldata, 'html.parser') 
        for data in soup.find_all("div", class_="variablelist"): 
            texts.append(data.get_text())
        for t in texts:
            if "Check ID" in t:                
                ...
                id_row = (x[2])
                checkid = id_row.replace("AWS Config ","").replace(" ","")
                fault_check_ids.append(checkid)                    
        output['fault-tolerance']=fault_check_ids
        #print(fault_check_ids)

        operation_check_ids = []
        texts = []
        url = 'https://docs.aws.amazon.com/awssupport/latest/user/operational-excellence-checks.html'
        htmldata = getdata(url)
        soup = BeautifulSoup(htmldata, 'html.parser') 
        for data in soup.find_all("div", class_="variablelist"): 
            texts.append(data.get_text())
        for t in texts:
            if "Check ID" in t:                
                ...
                id_row = (x[2])
                checkid = id_row.replace("AWS Config ","").replace(" ","")
                operation_check_ids.append(checkid)                    
        output['operational-excellence']=operation_check_ids
        #print(operation_check_ids)

        servicelimit_check_ids = []
        texts = []
        url = 'https://docs.aws.amazon.com/awssupport/latest/user/service-limits.html'
        htmldata = getdata(url)
        soup = BeautifulSoup(htmldata, 'html.parser') 
        for data in soup.find_all("div", class_="variablelist"): 
            texts.append(data.get_text())
        for t in texts:
            if "Check ID" in t:                
                ...
                id_row = (x[2])
                checkid = id_row.replace("AWS Config ","").replace(" ","")
                servicelimit_check_ids.append(checkid)                    
        output['service-limits']=servicelimit_check_ids
        result = output
...
def get_all_trusted_advisor_checks():
    ...
    try:
        ...
        results.extend(response['checkSummaries'])
...
output = []
ta_check_list_public_doc = get_all_trusted_advisor_checks_in_public_doc()
ta_checks_list = get_all_trusted_advisor_checks()
wata_list = gather_wellarchitected_ta_mapping()

for tacheck in ta_checks_list:
    
    #Checking if in public doc
    in_pub_doco = False
    for i in ta_check_list_public_doc['service-limits']:
        if i == tacheck['id']:
            in_pub_doco = True
            
    for i in ta_check_list_public_doc['operational-excellence']:
        if i == tacheck['id']:
            in_pub_doco = True

    for i in ta_check_list_public_doc['fault-tolerance']:
        if i == tacheck['id']:
            in_pub_doco = True

    for i in ta_check_list_public_doc['security']:
        if i == tacheck['id']:
            in_pub_doco = True

    for i in ta_check_list_public_doc['performance']:
        if i == tacheck['id']:
            in_pub_doco = True

    for i in ta_check_list_public_doc['cost-optimization']:
        if i == tacheck['id']:
            in_pub_doco = True
    ...
    mapping=[]
    for watachecks in wata_list:
        if tacheck['id'] == watachecks['TrustedAdvisorCheckId']:
            mapping.append(watachecks['WABestPracticeId'])
        
    output.append(
        {
            'TACheckId': tacheck['id'],
            'TACheckCategory': tacheck['pillars'][0],
            'TACheckWABPMapping' : mapping,
            'TACheckInPublicDoco' : in_pub_doco
        })
...
csv_writer.writerow(['TACheckId','TACheckCategory','TACheckWABPMapping','TACheckInPublicDoco'])

for item in output:
    csv_writer.writerow([item['TACheckId'], item['TACheckCategory'],item['TACheckWABPMapping'],item['TACheckInPublicDoco']])
